...
export default function Settings() {
  ...
  return (
    <div>
      ...
      {/* Remount (re-seeding every field from props) whenever the profile changes. */}
      <SettingsForm key={data.user.id} data={data} role={role} />
    </div>
  )
}

function SettingsForm({ data, role }: { data: FullProfile; role: Role }) {
  ...
  // student
  const [studentId, setStudentId] = useState(data.student?.student_id ?? '')
  const [sDept, setSDept] = useState<Department | ''>(
    (data.student?.department ?? '') as Department | '',
  )
  const [yearLevel, setYearLevel] = useState(data.student?.year_level ?? '')
  const [major, setMajor] = useState(data.student?.major ?? '')
  // instructor
  const [iDept, setIDept] = useState<Department | ''>(
    (data.instructor?.department ?? '') as Department | '',
  )
  const [category, setCategory] = useState(data.instructor?.category ?? '')
  const [office, setOffice] = useState(data.instructor?.office_location ?? '')
  const [bio, setBio] = useState(data.instructor?.bio ?? '')
  const [specializations, setSpecializations] = useState(
    data.instructor?.specializations ?? '',
  )
